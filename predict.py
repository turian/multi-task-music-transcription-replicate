...
class Model(cog.Model):
    ...
    @cog.input("model_type", type=str, options=["ismir2021", "mt3"], default="mt3", help="Model type: ismir2021 for piano, mt3 for multi-instrument")
    def predict(self, audio_file, model_type):
        self.setup(model_type)
        audio, sr = note_seq.audio_io.wav_data_to_samples_librosa(audio_file.read(), sample_rate=16000)
